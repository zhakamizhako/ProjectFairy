...
public class CatchArmController : UdonSharpBehaviour {
    ...
    void Update () {
        if (Detector != null) {
            ...
            if (selectedIndex != -1) {
                if (localPlayer != null && Current_LSC!=null && Current_LSC.EngineControl!=null && Current_LSC.EngineControl.Occupied == false && Networking.IsOwner (gameObject)) { // in the event that a person has left the plane while snagged.
                    isReady = false;
                    isCaught = false;
                    isLaunched = false;
                    isPulling = false;
                ...
                if (isReady) {
                    ...
                    if (isCaught) {
                        ...
                        AnimationControl.SetBool ("arm_caught", true);
                        if (Current_LSC != null) { // checker for current LSC
                            TrackerObject.isRendered = false;
