...
public class EffectsController : UdonSharpBehaviour {
    ...
    [System.NonSerializedAttribute][UdonSynced (UdonSyncMode.None)] public bool Tarps = false;
    ...
    public void Effects () {
        ...
        if (Flaps) { PlaneAnimator.SetBool ("flaps", true); } else { PlaneAnimator.SetBool ("flaps", false); }

        if (Tarps) { PlaneAnimator.SetBool ("tarps", true); } else { PlaneAnimator.SetBool ("tarps", false); }

        if (GearUp) { PlaneAnimator.SetBool ("gearup", true); } else { PlaneAnimator.SetBool ("gearup", false); Tarps = false; }

        if (HookDown) { PlaneAnimator.SetBool ("hookdown", true); } else { PlaneAnimator.SetBool ("hookdown", false); }

        if (AfterburnerOn) { PlaneAnimator.SetBool ("afterburner", true); } else { PlaneAnimator.SetBool ("afterburner", false); }
        ...
        PlaneAnimator.SetFloat ("throttle", EngineControl.Throttle);
