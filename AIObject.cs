        ...
        if (updateTimer > updateTendency) {
            ...
            if ((Networking.IsOwner (gameObject) || localPlayer == null) && disabled == false && dead == false) {
                if (TargetDetector != null) {
                    RaycastHit[] hit = Physics.SphereCastAll (TargetDetector.position, radius, TargetDetector.forward, 5000, TargetingLayermask, QueryTriggerInteraction.UseGlobal); //in case of shit happens like multiple rayhitted objects
                    if (hit.Length > 0) {
    ...
    void MovementLogic () {
        if (!dead) {
            if (type == "air") {
                ...
                if (shouldAttack && !dontchaseTarget && debugTargets.Length > 0) { //Attack Logic
                ...
                if ((!shouldAttack || (shouldAttack && dontchaseTarget)) && Waypoints != null && Waypoints.Length > 0 && FollowObject == null) { // Waypoint Logic
                    shouldFollowLeft = false;
                    shouldFollowRight = false;
                    ...
                    if (shouldGoNextWaypoint) { // if false, circle around the waypoint. 
                ...
                } else if (!shouldAttack && FollowObject && !dontchaseTarget) { //Follow Object Logic
                    if (shouldFollowLeft == false && shouldFollowRight == false) { //decide whether to follow left or right
